...
inline int maximum(PRIORITY_QUEUE *priority_queue) {
    return heapMinimum(priority_queue);
}

inline int extractMinimum(PRIORITY_QUEUE *priority_queue) {
    return heapExtractMinimum(priority_queue);
}

inline void increaseKey(PRIORITY_QUEUE *priority_queue, int index,
                        int key) {
    return heapIncreaseKey(priority_queue, index, key);
}

inline void insert(PRIORITY_QUEUE *priority_queue, int key) {
    return minHeapInsert(priority_queue, key);
}
